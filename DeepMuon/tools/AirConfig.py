...
LastEditTime: 2023-01-28 13:48:42
Description: NULL
...
class Config:
    ...
    def __para_config(self):
        ...
        if 'filepath' not in model_info.keys() or not os.path.exists(model_info['filepath']):
            self.paras['model'] = {
                'backbone': internal_env[self.config.model['backbone']], 'params': model_params}
        else:
            self.paras['model'] = {'backbone': getattr(import_module(
                model_info['filepath']), model_info['backbone']), 'params': model_params}
        ...
        if self.config.loss_fn is None:
            ...
            if 'filepath' not in loss_info.keys() or not os.path.exists(loss_info['filepath']):
                ...
                self.paras['loss_fn'] = {
                    'backbone': internal_env[self.config.loss_fn['backbone']], 'params': loss_fn_para}
            else:
                if 'params' not in loss_info.keys():
                    loss_fn_para = dict()
                else:
                    loss_fn_para = loss_info['params']
                self.paras['loss_fn'] = {'backbone': getattr(import_module(
                    loss_info['filepath']), loss_info['backbone']), 'params': loss_fn_para}
        ...
        if 'filepath' not in optimizer_info.keys() or not os.path.exists(optimizer_info['filepath']):
            ...
            self.paras['optimizer'] = dict(bakcbone=getattr(import_module(optimizer_info['filepath']), optimizer_info['backbone']),
                                           params=optimizer_info['params'])
        schedular_info = self.config.schedular
...
# config = Config(
#     configpath='/Users/airskcer/Library/CloudStorage/OneDrive-USTC/Muon/DeepMuon/config/Hailing/CSPP.py')
# print(config)
# env=globals()
# keys=list(env.keys())
