...
LastEditTime: 2023-01-03 16:26:13
Description: NULL
...
class Config:
    ...
    def __para_config(self):
        ...
        if 'filepath' not in model_info.keys() or not os.path.exists(model_info['filepath']):
            self.paras['model'] = {
                'backbone': info[self.config.model['backbone']], 'params': model_params}
        else:
        ...
        if self.config.loss_fn is None:
            ...
            if 'filepath' not in loss_info.keys() or not os.path.exists(loss_info['filepath']):
                ...
                self.paras['loss_fn'] = {
                    'backbone': info[self.config.loss_fn['backbone']], 'params': loss_fn_para}
            else:
                if 'params' not in loss_info.keys():
                    loss_fn_para = dict()
                else:
