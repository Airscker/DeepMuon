...
def add_random_number(img:np.ndarray):
...
def rotate(img:np.ndarray,angle_range=180):
...
def bright(img:np.ndarray,light_range=(0.5,1.2)):
...
class EGFR_NPY(Dataset):
    def __init__(self,
        ...
        with open(img_dataset,'rb')as f:
            '''list([imgs,label,pid]), imgs.shape=(3,224,224)'''
    ...
    def __getitem__(self, index):
        img=self.clip_top_bottom(self.dataset[index][0])
        img=self.norm_range(img)
        if self.augment:
            env=globals()
            for augment in self.augment_pipeline:
                try:
                    img = env[augment['type']](img, **exclude_key(augment))
                except:
                    pass
