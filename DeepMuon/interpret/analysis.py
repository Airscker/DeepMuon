...
def loss_dist(loss, title='Loss', bins=15, sigma=3, save='', show=False):
    plot_hist_2nd(loss, title=title, bins=bins,
                  sigma=sigma, save=save, show=show)
...
def data_analysis(thres, sigma, config, fold=0.8):
    ...
    logger = LOGT(log_dir=os.path.join(work_dir, 'ana'),
                  logfile='ana.log', new=False)
    logger.log(
    ...
    logger.log(
        f'Distribution Histogram Image of Loss value is saved as {name}')
    name = os.path.join(
        work_dir, 'ana', f'loss_sigma_{sigma}_thres_{thres}.jpg')
    thres_dist(loss, thres=thres, save=name)
    logger.log(
        f'Distribution Histogram Image of Loss value Threshold {thres} is saved as {name}')
    # Plot loss/lr curve
    train_data = load_log(os.path.join(
        work_dir, train_config.paras['work_config']['logfile']))
    ...
    plot_curve(data=[test_loss, train_loss], title='Loss Value Curve', data_label=[
               'Test Loss', 'Train Loss'], save=curve_path)
    logger.log(f'Loss Value Curve is saved as {curve_path}')
    curve_path = os.path.join(
        work_dir, 'ana', f'TR_TS_loss_f_{fold_index}.jpg')
    plot_curve(data=[test_loss[fold_index:], train_loss[fold_index:]],
               title=f'Loss Value Curve Start From Fold_{fold} Index: {fold_index}', data_label=['Test Loss', 'Train Loss'], save=curve_path)
    logger.log(
        f'Loss Value Curve Start From Fold_{fold} Index: {fold_index} is saved as {curve_path}')
    curve_path = os.path.join(work_dir, 'ana', 'lr_curve.jpg')
    plot_curve(data=train_data[:, 0], title='Learn Rate', data_label=[
               'Learn Rate'], axis_label=['Epoch', 'LR'], save=curve_path)
    logger.log(f'Learn rate curve during training is saved as {curve_path}')
    # Log key information of loss
    sigma_range = [np.mean(loss)-sigma*np.std(loss),
                   np.mean(loss)+sigma*np.std(loss)]
    thres_loss = loss[loss < thres]
    sigma_range_thres = [np.mean(
        thres_loss)-sigma*np.std(thres_loss), np.mean(thres_loss)+sigma*np.std(thres_loss)]
    logger.log(f'{sigma}Sigma range: {sigma_range}')
    logger.log(
        f'{sigma}Sigma range of threshold value {thres}: {sigma_range_thres}')
    logger.log(f'Total number of samples: {len(loss)}')
    logger.log(
        f'Total number of threshold {thres} resampled samples: {len(thres_loss)}')
