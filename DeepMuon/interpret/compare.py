...
def loss_com(work_dirs, output='', max=0.2, num=0):
    ...
    assert os.path.isdir(
        output), f'Please specify the root path of output,rather then {output}'
    ...
    plot_curve(data=tsl, title='Compare testing performance of several models',
               data_label=name, save=ts_name)
    plot_curve(data=trl, title='Compare training performance of several models',
               data_label=name, save=tr_name)
    print(
        f'The compare curve is saved as:\nTraining: {tr_name}\nTesting: {ts_name}\nCompared Model work_dirs: {name}')
    return tsl, trl, name
